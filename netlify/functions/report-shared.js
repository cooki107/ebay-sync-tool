// Shared logic used by send-daily-report.js, send-weekly-report.js and
// send-monthly-report.js - order fetching, HTML building, and sending are
// identical across all three; only the date range and some wording differ.
...
// (ISO 8601 strings). Used directly by the weekly/monthly reports, and by the
// daily report's own startTime/endTime calculation (which has its own
// weekend catch-up logic).
async function getSalesForRange(startTime, endTime, authToken, appId, devId, certId, hostname) {
...
function buildEmailHtml(sales, traffic, costs, options) {
  ...
  const preheaderText = sales.length > 0
    ? `${totalItems} item${totalItems === 1 ? '' : 's'} sold for £${totalRevenue.toFixed(2)} ${preheaderPeriod}`
    : noSalesPreheader;

  return `
  <html><body style="font-family:-apple-system,sans-serif;background:#f8fafc;padding:20px;">
    <!-- Hidden preheader - the text inbox previews show next to the subject line -->
    ...
    <div style="max-width:600px;margin:0 auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 10px 40px rgba(0,0,0,0.08);">
      <div style="background:#0a0a0a;padding:32px 20px;text-align:center;color:white;">
        ...
        <h1 style="margin:0;font-size:26px;">${reportTitle}</h1>
        <p style="margin:4px 0 0;opacity:0.9;font-size:13px;color:#6bbdae;">NGLH Trading${dateRangeLabel ? ` &middot; ${dateRangeLabel}` : ''}</p>
      </div>
      <div style="padding:32px 20px;">
            ...
            <td width="50%" style="padding-left:6px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#fef3c7;border:1px solid #fecaca;border-radius:12px;">
                <tr><td style="padding:14px;text-align:center;">
                  ...
                  <div style="font-size:11px;color:#d97706;font-weight:600;text-transform:uppercase;margin-top:4px;">Views</div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:24px;">
          <thead><tr>
            <th style="text-align:left;padding:8px;font-size:11px;color:#64748b;">Game</th>
            <th style="text-align:right;padding:8px;font-size:11px;color:#64748b;">Views</th>
          </tr></thead>
      ...
      <div style="background:#f8fafc;border-top:1px solid #e2e8f0;padding:20px;text-align:center;font-size:12px;color:#94a3b8;">
        NGLH Trading • eBay Sync • ${footerCadence}
      </div>
    </div>
  </body></html>`;
}

async function sendViaResend(htmlContent, resendApiKey, toEmail, subject) {
