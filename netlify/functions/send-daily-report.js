...
function buildEmailHtml(sales, traffic) {
  ...
  const salesRows = sales.length > 0 ? sales.map((s, i) => `
    ...
    </tr>`).join('') : `<tr><td colspan="4" style="padding:20px 8px;text-align:center;color:#94a3b8;">No sales recorded yesterday.</td></tr>`;
  ...
  const preheaderText = sales.length > 0
    ? `${totalItems} item${totalItems === 1 ? '' : 's'} sold for £${totalRevenue.toFixed(2)} yesterday`
    : 'No sales yesterday - nothing to report today';
